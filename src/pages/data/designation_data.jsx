...
function DesignationData() {
	const [data, setData] = useState({
		designations: [], // Initialize with an empty array
	});
	...
	}, [loading, data.designations.length]);
	...
	const handleFormSubmit = () => {
		if (!newDesignation.trim()) {
			setErrors('Designation cannot be empty');
			return;
		}

		// Update the list of designations
		setData((prevData) => {
			const updatedDesignations = [...prevData.designations, newDesignation.trim()];
			console.log('list_of_strings:', updatedDesignations); // Print the entire list to the console
			axios
			...
			return { ...prevData, designations: updatedDesignations };
		});
			...
			>
				...
				>
					{data.designations.map((designation, index) => (
						<UserListItem key={index} designation={designation} />
					))}
				</List>
			</Box>
...
function UserListItem({ designation }) {
	return (
